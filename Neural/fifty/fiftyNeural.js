...
import {weights} from "../src/weightsForFifty.js";
...
//initialization
    function initialization() {
        ...
        //weightsArr init
        weightsArr = new Array(layersCount - 1);
        setWeights();
        //bias init
    ...
    function setWeights() {
        for (let i = 0; i < layersCount - 1; i++) {
            weightsArr[i] = new Array(vectorSize[i + 1]);
            for (let j = 0; j < vectorSize[i + 1]; j++) {
                weightsArr[i][j] = new Array(vectorSize[i]);
                for (let k = 0; k < vectorSize[i]; k++) {
                    weightsArr[i][j][k] = getRandom(-1, 1);
                }
            }
        }
    }
...
// main algorithms
    function forwardFeed() {
        for (let i = 1; i < layersCount; i++) {
            let temp = matrixMultiply(weightsArr[i - 1], neuronValues[i - 1]);
